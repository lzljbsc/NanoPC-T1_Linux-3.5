...
7、arch/arm/lib/memset.S 文件对代码的影响；    ————linux内核patch中，有关于对
memset.S 的补丁，是因为 gcc-4.7.2 以后的版本，优化了对函数调用后的返回值的使用，
而之前的 memset 函数并无返回值，导致使用新的 gcc 会出现内存访问错误；
...
26、devtmpfs 用于 /dev 目录管理，开启 devtmpfs 文件系统后，在使用 device_add 创
建设备时，会自动在 /dev 目录下创建设备节点； mdev 机制也是可以同时使用，两个互补；
devtmpfs 创建的设备节点默认属性为 0600, mdev 机制在设备属性、访问权限、用户组等
可以进一步补充；
...
USB PHY HSIC 的功能；
34、更新编译器版本后，启动时总是出现 panic 问题，出现的位置是一些循环、指针操作，
更新了 arch/arm/lib/memset.S 文件后解决，该文件中具体操作未分析；
35、将内核 DEBUG_LL 选项默认关闭，即不使用 early_print 功能；
36、开启 MTD子系统，并开启了 mtdram / nandsim 两个mtd设备，在 bootargs 中添加了
分区信息；
37、同步了原在 iTop4412 3.0.15 中的很多代码注释，并解决了一些编译警告问题；
38、开启 devtmpfs ，启动后自动挂载；
39、移除 CONFIG_LOCALVERSION_AUTO 配置项，不自动添加自动版本号；
    添加自动版本号可以追踪内核版本信息，如分支、提交hash等，方便问题定位；
    但添加了之后，在内核模块加载时会出现找不到指定目录的问题，因为内核模块加载默
    认目录按照内核版本全名查找的； 这里就默认不加了，方便调试；
40、开启 input 子系统支持，添加了两个 GPIO 按键，通过 evtest 工具可测试按键按下/
抬起状态；
41、开启 media 的 ir 检测功能，已适配板上红外器件；
    注意，将 active_low 设置为 1。
42、开启 i2c 功能，/dev 下有 i2c 总线节点；
