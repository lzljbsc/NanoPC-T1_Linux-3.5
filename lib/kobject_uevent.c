		       ...
		       char *envp_ext[])
{
    ...
    /* 从需要发送 uevent 的 kobj 开始判断是否有所属的 kset
     * 如果没有所属的 kset, 那就找 kobj 的 父parent,
     * 直到直到所属的 kset 或 没有父parent 了
     * 两种情况，要么找到了 kset
     * 要么没有父了，没有父就说明已经在顶层了
     * 不属于 kset ，并且没有父的， devices_kset 就是一个
     * devices_kset 是一个 /sys 顶层目录项 */
	top_kobj = kobj;
	while (!top_kobj->kset && top_kobj->parent)
		top_kobj = top_kobj->parent;

    /* 只有属于 kset 才能够发送 uevent ，不属于任何 kset 则报错 */
    /* 不属于任何 kset 的 kobject 是允许存在的，只是一个单纯的目录
     * 这种 kobject 不能发送 uevent */
	if (!top_kobj->kset) {
