...
static void __init cacheid_init(void)
{
    ...
    /* iTop4412:  CPU: VIPT nonaliasing data cache, VIPT aliasing instruction cache */
    /* NANOPC-T1: CPU: PIPT / VIPT nonaliasing data cache, VIPT aliasing instruction cache */
...
int __init arm_add_memory(phys_addr_t start, unsigned long size)
{
     ...
     * */
    /* NANOPC-T1 开发板实测打印结果
     * arm_add_memory: start: 0x40000000  size: 0x10000000
     * arm_add_memory: start: 0x50000000  size: 0x10000000
     * arm_add_memory: start: 0x60000000  size: 0x10000000
     * arm_add_memory: start: 0x70000000  size: 0x0ff00000
     * */
...
static struct machine_desc * __init setup_machine_tags(unsigned int nr)
{
    ...
    /* 下面的一些分析，是按照 iTop4412 开发板分析的
     * 在 NanoPC-t1 开发中， __atags_pointer != 0 */
    /* __atags_pointer 本代码中为 0
     * 走 else 分支 */
    /* 这里重点说明一下 else 分支中的处理
     * 在 3.0.15 的版本中，else 处理了比较多的内容，判断了参数在
     * 内存中的位置（必须在 前 1M以内），但在后面的版本中（如 3.5
     * 4.14 5.4 ）都是下面这种形式了，不判断地址的范围了
     * 这里附一下 3.0.15 中的代码
     *
		if (__atags_pointer)
...
void __init setup_arch(char **cmdline_p)
{
    ...
    /* setup_arch->machine_arch_type : 4608
     * 这里的 machine_arch_type = 4608 */
	...
	*cmdline_p = cmd_line;
    ...
    /* 板级的早期初始化，在 mach-nanopc-t1.c 中定义了
     * 用于初始化 时钟系统和控制台串口 */
	if (mdesc->init_early)
		mdesc->init_early();
}
