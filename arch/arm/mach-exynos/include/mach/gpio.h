...
/* 这里默认使用的是 EXYNOS5_GPIO_END 作为 GPIO_END 
 * 但在适配时，为了尽量减少对源码的修改，并未将 exynos4 中原来的移除，
 * 如 GPE 这些gpio，这样，就导致了 gpio 号往后排了，导致过大
 * 而在注册gpiochip时，是需要判断注册的gpio号和预留的总数量
 * 原来预留的数量小了，所以就改为了 EXYNOS4_GPIO_END 作为 GPIO_END
 * 根据手册，EXYNOS4412 总共有 304个gpio， 这里总共预留了 330个 */
